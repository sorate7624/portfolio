...
import { actozsoft } from "../assets";
import { SectionWrapper } from "../hoc";
import { projects } from "../constants";
import { fadeIn, textVariant } from "../utils/motion";

const ProjectCard = ({ index, name, description, tags, image, url, act }) => {
  return (
    <motion.div variants={fadeIn("up", "spring", index * 0.3, 0.6)}>
      <div className="flex flex-col p-5 rounded-2xl sm:w-[350px] max-w-xs w-full h-[500px] bg-pastel-navy">
        <a href={url} target="_blank" className="relative w-[full] h-[150px]">
          <img
            src={image}
            alt={name}
          ...
          <div className="absolute inset-0 flex justify-end m-3 card-img_hover">
            <span className="w-10 h-10 rounded-full flex justify-center items-center bg-white border-4 border-[#19376D] shadow-2xl">
              {act ? (
                <img
                  src={actozsoft}
                  alt="actozsoft"
        ...
        <div className="mt-5">
          ...
          <p className="mt-2 text-[14px] whitespace-pre-wrap break-words">
            {description}
          </p>
        </div>
