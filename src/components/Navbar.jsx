...
const Navbar = () => {
  ...
  const navHighlighter = () => {
    ...
    document.querySelectorAll("nav li").forEach((list) => {
      list.classList.remove("text-purple");
    });
    ...
    sections.forEach(current => {
      ...
      if (
        ...
        document.querySelector("nav li a[href*=" + sectionId + "]").classList.add("text-purple");
      } else {
        document.querySelector("nav li a[href*=" + sectionId + "]").classList.remove("text-purple");
      }
    });
  };
      ...
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        <ul className="list-none hidden sm:flex flex-row gap-10 abcd">
          {navLinks.map((nav) => (
            ...
            >
              <a href={`#${nav.id}`} className="hover:text-purple">{nav.title}</a>
            </li>
          ))}
        </ul>
